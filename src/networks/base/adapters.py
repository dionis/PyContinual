...
class BertAdapterMask(BertAdapter):
    ...
    def mask(self,t,s=1):
       ...
       efc1 = self.efc1(torch.LongTensor([t]).cuda() if  torch.cuda.is_available() else torch.LongTensor([t]) )
       efc2 = self.efc2(torch.LongTensor([t]).cuda() if  torch.cuda.is_available() else torch.LongTensor([t]))
    ...
    def __init__(self, config,layer_type):
        ...
        if layer_type=='semantic':
            ...
            if  config.no_tsv_mask:
                self.tsv = torch.ones( config.ntasks,config.ntasks).data.cuda()  if torch.cuda.is_available()  else  torch.ones( config.ntasks,config.ntasks)# for backward
            else:
                self.tsv = torch.tril(torch.ones(config.ntasks,config.ntasks)).data.cuda() if torch.cuda.is_available()  else  torch.tril( torch.ones(config.ntasks,config.ntasks))# for backward
...
class CapsuleLayer(nn.Module): #it has its own number of capsule for output
    ...
    def forward(self, t,x,s,layer_type=None):
        ...
        if layer_type=='tsv':
            batch_size = x.size(0)
            priors = x[None, :, :, None, :] @ self.route_weights[:, None, :, :, :]
            logits = torch.zeros(*priors.size()).cuda()  if torch.cuda.is_available()  else torch.zeros(*priors.size())
            # print('logits: ',logits.size())  torch.Size([3, 32, 19, 1, 128])
            mask = torch.zeros(self.config.ntasks).data.cuda()   if torch.cuda.is_available()  else torch.zeros(self.config.ntasks)
