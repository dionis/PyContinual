...
class Net(torch.nn.Module):
    ...
    def mask(self,t,s=1):
        gc1=self.gate(s*self.ec1(torch.LongTensor([t]).cuda() if torch.cuda.is_available() else torch.LongTensor([t]) ))
        gc2=self.gate(s*self.ec2(torch.LongTensor([t]).cuda() if torch.cuda.is_available() else torch.LongTensor([t]) ))
        gc3=self.gate(s*self.ec3(torch.LongTensor([t]).cuda() if torch.cuda.is_available() else torch.LongTensor([t]) ))
